...
# DirectML backward broken, use CPU when CUDA unavailable
...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000):
    ...
    cpu_buffer = torch.empty(
        2 * B * T, dtype=torch.long, pin_memory=(_DEVICE_STR == "cuda")
    )
    gpu_buffer = torch.empty(2 * B * T, dtype=torch.long, device=_DEVICE_STR)
...
def evaluate_bpb(model, tokenizer, batch_size):
    ...
    """
    token_bytes = get_token_bytes(device=_DEVICE_STR)
